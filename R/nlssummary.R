nlssummary <-
function (g,N,mc,studentize,center,cbb) {
    wr1<- g$x-g$pred.x
    wr2<- g$y-g$pred.y
    if (center==TRUE) {
      wr1 <- wr1 - mean(wr1) 
...
if (mc > 1) {
  ...
  bootdat <- clusterMap(cl,nlsbootwrapper, j=1:N, MoreArgs=list(wr1=wr1,wr2=wr2,x.pred=g$pred.x,y.pred=g$pred.y,n=n,cbb=cbb),SIMPLIFY=TRUE)
  stopCluster(cl)
  }
  else  bootdat <- mapply(nlsbootwrapper, j=1:N, MoreArgs=list(wr1=wr1,wr2=wr2,x.pred=g$pred.x,y.pred=g$pred.y,n=n,cbb=cbb))
    ...
    if (warningcount > 0) warning("The function nls failed to converge ",warningcount," times.")
