...
if (mc > 1) {
  ...
  bootdat <- clusterMap(cl,lmbootwrapper, j=1:N, MoreArgs=list(wr1=wr1,wr2=wr2,x.pred=g$pred.x,y.pred=g$pred.y,n=n,cbb=cbb),SIMPLIFY=TRUE)
  stopCluster(cl)
  }
  else  bootdat <- mapply(lmbootwrapper, j=1:N, MoreArgs=list(wr1=wr1,wr2=wr2,x.pred=g$pred.x,y.pred=g$pred.y,n=n,cbb=cbb))
...
if (warningcount > 0) warning("Model failed to run ",warningcount," times.")
